 ...
 */
function runCueConfigInit(options) {
 ...
 */
function runCueConfigGenerate(options) {
    return (_tree, context) => {
        const rootPath = resolveRoot(options.rootPath ?? "~/git");
        const packagePath = join(rootPath, options.name);
 ...
 */
function createGitHubRepo(options) {
    return (_tree, context) => {
        if (options.skipGit) {
 ...
 */
function addToManifest(options) {
    return (tree, context) => {
 ...
 */
export function libNew(options) {
    return chain([
